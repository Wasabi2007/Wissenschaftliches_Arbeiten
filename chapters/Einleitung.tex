...
\chapter {Dijkstra}
Dijkstra [?d??k?stra]bildet die Grundlage zu A*. Er wurde vom gleichnamigen Professor im Jahr 1959 in einem Lehrbuch erläutert.\footnote{\cite{EWD:NumerMath59}} Es ist der üblichste Algorithmus, der sowohl für Netwerkrouting eingesetzt wird, als auch für Kartennavigationsgeräte.\footnote{\cite{tanenbaum:CN}} \\
...
\begin{enumerate}
	...
	\begin{itemize}
		\item War dies der Zielknoten, so hat man als Ergebnis den Weg vom Anfang bis zum Ziel führt.
...
\chapter{A*}
Der Algorithmus von Dijkstra stellte sich als zufriedenstellend für Karten und Netzwerkverkehr heraus. Aber für die Anforderungen, die KI-Entwickler ende der Sechziger Jahre zu bewältigen hatten war er einfach nicht geeignet. Dijkstra ist klar im Vorteil wenn eine Strecke über bekannte Punkte mit konstanten Längen zurück gelegt werden soll. Aber sobald es an ein Raster geht, an dem der -nur theoretisch vorhandene- Abstand der Kacheln immer gleich ist, kann das Verfahren optimiert werden.
A* ist demnach kein komplett neues Verfahren, sondern nur eine Optimierung des Ursprünglichen Algorithmus hinsichtlich dieser Punkte.
